...
from src.utils.s3_utils import read_csv_from_s3_to_df, upload_df_as_parquet_to_s3, generate_s3_partitioned_key, list_objects, get_target_s3
...
def run(s3_key: str, write_back_to_s3: bool = False):
    logger.info("Starting call logs data extraction: %s", s3_key)

    if write_back_to_s3: 
        s3_target_key = generate_s3_partitioned_key("call-log", s3_key)
